...
int   fuerza_cluster(int *red,int n, int *punt_n);
...
int main(int argc,char *argv[]) //Via terminal le tengo que pasar el valor de cada argumento
{
  ...
  printf("Program takes an average of %g seconds\n",diff_t);
...
int   fuerza_cluster(int *red,int n, int *nodos_perc)
{
	int i,j,etiq[10],a,etiqueta,ij,b;
	int f1[n];
	int fn[n];
	a = 0; 
	etiqueta = 0;       

	for (i=0;i<10; i++) *(etiq+i)=0;
	...
	for(i=0;i<n;i++)
	{
		for(j=0;j<n;j++)
		{
			if (f1[i]==fn[j] && f1[i]!=0)	//me fijo si una etiqueta del extremo superior es igual a una del inferior
			{
				etiqueta=f1[i];
				if (a==0) 
				{
					*etiq=f1[i]; //guardo la etiqueta del cluster percolante
					a++;
				}
				...
				{
					b=0;
					for (ij=0;ij<a;ij++) //se fija si esa etiqueta se repite (por si hay mas de dos clusters percolantes)
					{
						if (etiqueta!=*(etiq+ij)) //se fija si es diferente a las anteriores etiquetas
						{
							b++; //acumulo. Si es distinto de todos las anteriores etiquetas entonces lo guardo
						}
					}
					if (b==a)
					{
						*(etiq+a)=f1[i]; //guardo la etiqueta del cluster percolante
						a++;
					}	
				}
	...
	for(i=0;i<n*n;i++) //voy barriendo la red y me fijo cuantos de ellos tienen la etiqueta "percolante"
	{
		for (ij=0; ij<a; ij++)
		{
			if (red[i]==*(etiq+ij) && *(etiq+ij)>0) 
			{
				*nodos_perc+=1;
			}
		}
	}
//printf("nodos_perco vale %i\n",nodos_perc);
//printf("\n\n");
//printf("%i",p);
