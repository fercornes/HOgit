...
#define Z     30000          // numero de iteraciones
...
double valor_medio(float z,int n);
double dispersion(float z,float x,int n);
...
int main(int argc,char *argv[]) //Via terminal le tengo que pasar el valor de cada argumento
{
  ...
  for(i=0;i<z;i++) //hago z iteraciones, es decir me "muevo" a través de la red infinita
  {
	...
	prob_cuadr_acum=prob_cuadr_acum+(float)(prob*prob);
  }
...
{
	float m,d,k;
	m=z/(float)n; //media de p
	d=x/(float)n; //media de p cuadrado
	k=sqrt(d-m*m);
	return k;
}
